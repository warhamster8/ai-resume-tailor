...
export default function ResumePDF({ data, templateId }: Props) {
  ...
  const styles = StyleSheet.create({
    ...
    eliteProfileBox: { backgroundColor: '#2d2d2d', paddingVertical: 25, paddingHorizontal: 30, color: '#fff', marginBottom: 35 },
    ...
    eliteProgressBar: { height: 3, backgroundColor: '#eee', marginTop: 5, flexDirection: 'row', overflow: 'hidden' },
    eliteProgressFill: { height: 3, backgroundColor: '#1a1a1a' },
  ...
  const EliteTemplate = () => (
    <Page size="A4" style={styles.pageElite}>
      <View style={styles.eliteContainer}>
        ...
        <View style={styles.eliteProfileBox} wrap={true}>
        ...
        <View style={styles.eliteBody}>
          <View style={styles.eliteLeftCol}>
            <Text style={styles.eliteSectionTitle} wrap={true}>Istruzione</Text>
            {displayData.education?.map((edu: any, i: number) => (
              <View key={i} style={{ marginBottom: 15 }} wrap={true}>
                <Text style={{ fontSize: 10, fontWeight: 'bold' }}>{edu.degree}</Text>
                <Text style={{ fontSize: 8.5, color: '#666' }}>{edu.school}</Text>
              </View>
            ))}
            <Text style={[styles.eliteSectionTitle, { marginTop: 25 }]} wrap={true}>Skills</Text>
            {displayData.skills?.map((s: any, i: number) => (
              <View key={i} style={styles.eliteSkillRow} wrap={true}>
          ...
          <View style={styles.eliteRightCol}>
            <Text style={styles.eliteSectionTitle} wrap={true}>Esperienza Lavorativa</Text>
        ...
        <Text style={styles.eliteGdpr} wrap={true}>Autorizzo il trattamento dei dati personali (D. Lgs. 196/2003 e GDPR 679/16).</Text>
      </View>
    </Page>
  );
      ...
      <View style={{ flexDirection: 'row', minHeight: '100%' }}>
        <View style={styles.sidebar}>
          ...
          {(displayData.skills || []).map((s, i) => (
            <Text key={i} style={{ fontSize: 9, marginBottom: 6 }} wrap={true}>• {s.name}</Text>
          ))}
        </View>
        <View style={styles.main}>
          <Text style={styles.sectionTitle} wrap={true}>Profilo Professionale</Text>
          <Text style={styles.standardText} wrap={true}>{displayData.personalInfo.summary}</Text>
          <Text style={[styles.sectionTitle, { marginTop: 25 }]} wrap={true}>Esperienza</Text>
          {(displayData.experience || []).map((exp: any, i: number) => (
    ...
    <Page size="A4" style={styles.pageBleed}>
      <View>
        ...
        <View style={styles.slateBody}>
          <Text style={styles.slateSectionTitle} wrap={true}>Profilo Professionale</Text>
          <Text style={styles.standardText} wrap={true}>{displayData.personalInfo.summary}</Text>
          
          <Text style={styles.slateSectionTitle} wrap={true}>Esperienza Lavorativa</Text>
          {(displayData.experience || []).map((exp: any, i: number) => (
            <View key={i} style={{ marginBottom: 20 }}>
          ...
          <Text style={styles.slateSectionTitle} wrap={true}>Competenze & Skill</Text>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            {(displayData.skills || []).map((s: any, i: number) => (
              <Text key={i} style={{ fontSize: 8.5, paddingVertical: 4, paddingHorizontal: 10, backgroundColor: '#f1f5f9', color: '#334155', borderRadius: 4, marginRight: 6, marginBottom: 6 }} wrap={true}>{s.name}</Text>
            ))}
          </View>
        </View>
      </View>
